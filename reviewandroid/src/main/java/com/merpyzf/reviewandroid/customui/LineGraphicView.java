...
public class LineGraphicView extends View {
	...
	private int marginTop = 20;// 距离顶部的值
	private int marginBottom = 60;// 距离底部的值
	...
	protected void onDraw(Canvas canvas) {
		...
		mPaint.setColor(res.getColor(R.color.colorAccent));
		mPaint.setStrokeWidth(dip2px(0.1f));
	...
	private void drawAllXLine(Canvas canvas) {
		for (int i = 0; i < spacingHeight + 1; i++) {
			...
			drawText(String.valueOf(averageValue * i), blwidh / 2, bheight
					- (bheight / spacingHeight) * i + marginTop, canvas);
		}
	}
	...
	private void drawAllYLine(Canvas canvas) {
		for (int i = 0; i < yRawData.size(); i++) {
			...
			canvas.drawText(xRawDatas.get(i), blwidh + (canvasWidth - blwidh)
					/ yRawData.size() * i, bheight + dip2px(26),mPaintText );// X坐标
	...
	/**
	 *
	 * @param yRawData :各个点的坐标值
	 * @param xRawData ：横坐标的值
	 * @param maxValue ：纵坐标的最大值
	 * @param averageValue：纵坐标的倍数值
	 */
